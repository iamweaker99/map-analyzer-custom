...
## [2026-08-14] clearing order decided: Hyp.2 first (impact x uncertainty ranking) | decision
- **Ranking (user-approved):** all 7 hypotheses scored on impact (on the measured result / ability to reproduce P > O > A) x uncertainty (that it works). Order: **Hyp.2** (join-boundary discontinuity — the only hypothesis measuring the workbook's defined A552 mechanism; entirely unmeasured, highest uncertainty) > **Hyp.1** (trajectory descriptors — synthesis branch, high/high, vectors.rs precedent) > **Hyp.3** (covering the 32 excluded rows — changes the evaluation set 53->85, medium uncertainty) > Hyp.5 (sub-question of Hyp.1, folded into its test design) > Hyp.4 (component fix, near-free) > Hyp.6 (conditional on per-pattern separation). Hyp.7 = terminal verdict by construction (its test is the failure of Hyp.1-6) — defines the loop's exit, not a test item.
- **Focus rule (user):** one hypothesis at a time; all others deferred until the active one resolves; phase-gated workflow unchanged (stop + notify after each phase).
- **Wiki:** clearing order section added to [[motor-metrics-oiah]] (ranked table + decision). Uncommitted — save ritual with the next session-end.

## [2026-08-14] session end | session
- **Session ended (user).** Resumed from `resume:2026-08-13` (recap-only -> authorized). Save ritual executed: 08-12/08-13 wiki diff reviewed, approved (scope A+B+C) and committed (`78406e5`, 22 files: [[motor-metrics-oiah]] + [[motor-model-requirements]] + [[difficulty-philosophy]] + [[orchestration-cache-improvements]] + handoffs 08-11/12/13 + index/log/hub/module updates + skills lint-timing amendment + evidence artifacts: success-criteria Excel, 51-test-run sample, OIAH md + dumps). Memory pointers updated.
- **Clearing order decided (user):** hypotheses ranked by impact x uncertainty (documented on [[motor-metrics-oiah]] § Clearing order) — **Hyp.2 first** (join-boundary discontinuity, the only hypothesis measuring the workbook's defined A552 mechanism), then Hyp.1, Hyp.3, Hyp.5 (folded into Hyp.1's design), Hyp.4, Hyp.6; Hyp.7 = terminal verdict. **One hypothesis at a time**; others deferred.
- **Section definition refined (user, quoted in Obs.14):** a description covers from the time at its row until the colour tag discontinues or meets a new colour tag. Dump-verified: every multi-row run's description sits on the FIRST row only; rows 323+324 both carry text (split S025/S026). **Consequence: all 32 no-value rows have measurable siblings -> section-level coverage 51/51** — the abrupt set S009/S011/S012/S028/S033 is fully measurable. Two Hyp.2 experiment gaps dissolved (per-row invisibility; aggregation unit).
- **Hyp.2 experiment gaps (12) listed in discussion** — no solutions given; pending resolution: definition gaps (minor-section unit, temporal-vs-spatial "broken", snap-window predicate), falsifier re-scope, HH1 fork, verdict gate, S003 (rows 56-57 accident tint, still in the 08-12 sample) drop decision.
- **Artifacts:** `Prototyping/51_test_run_sample_readable.md` (chronological section view for user's workbook check — user will verify S003 and the 323/324 split).
- **Wiki:** handoff [[2026-08-14-handoff]] created; [[2026-08-13-handoff]] -> resolved; [[motor-metrics-oiah]] (clearing order + Obs.14), index updated. Uncommitted — save ritual pending next session review. Resume token: `resume:2026-08-14`.

## [2026-08-14] wiki lint (session-end run #8) | validate
- **Mechanical checks green:** 46 files scanned; 0 broken wikilinks (~460 link occurrences), 0 orphans, index drift clean both directions (index.md self-exempt root), frontmatter 0 violations (new handoff 08-14 valid: type session / status open / sources present).
- **Page cap — 1 flag (judgment call):** [[motor-metrics-oiah]] at 444 raw / **351 non-empty** (was 424/336 at run #7; +clearing order section + Obs.14 amendment this session). Still under the soft ~400 substantive cap, but growing with the clearing loop — split candidate if the loop adds more (extract Obs/Inf/Asp/Hyp or the reasoning map to sub-pages). Left as-is this run.
- **This session's additions checked:** handoff 08-14 (new, all links resolve), [[motor-metrics-oiah]] edits (clearing order table + Obs.14 rule — wikilinks resolve), index (08-13 line updated, 08-14 line added) — no drift.
- **Judgment items carried over (unchanged, pending user as before):** finger-control.md:72 link nit under stale banner; rhythm-segmentation 0.5-vs-0.35 historical sections; reading-hub reroute to stale pages.

## [2026-08-14] Hyp.2 experiment formed | session
- **Resumed from `resume:2026-08-14` (recap-only -> authorized).** Scope directive (user): Hyp.2 experiment formation discussion only; uncommitted diff + deferred items out of scope (remind at next resume — carried in handoff).
- **Workbook-check decisions (user):** S003 (rows 56-57, lighter orange tint) KEPT as orange — accidental shade of the same orange, not a different swatch (Obs.1 amended); S025/S026 (rows 323/324, both "flatten wiggle") genuinely distinct sections that look similar — split stands (Obs.14 amended).
- **Gaps resolved (user-dictated):** 1/2/9 by the section rule (per-row invisibility, control-labeled-by-absence, aggregation unit); 4 measure spikes on the existing pattern segmentation — no minor-section classification, transitions across row/pattern boundaries + section edges (S028 jump->stream, S029 exit jump); 5 "broken" is spatial — notes stay on the same grid unit, velocity spike is the mechanism, angle secondary; 6 snap window = rhythm grid (1/2, 1/4, 1/6) from map timing (external, circularity removed), spike = sign(Δv) flip at transition k (peaks AND valleys both count) vs local baseline, threshold DATA-DERIVED (e.g. MAD-type outlier; user rejected a fixed assigned value — exact form pinned in spec); 7/8 counterpart channels — Hyp.2 (velocity spike) vs Hyp.1 (geometry), valid subjects = spike-bearing sections only, HH1 fork collapsed (angle = secondary weight); 10 answered by `Prototyping/51_test_run_sample.json` (all 51 sections, one source workbook, row->pattern->note-range->snap complete; coordinates in `yoasobi_collab_extra_motor_patterns.json`); 11 delegated (range indices <-> motor-JSON note ordering fidelity at execution).
- **Test set (user):** the full 10-row spike set — S009/S011/S012/S028/S033 (P), S010/S029/S030/S042 (O), S051 (A); membership per workbook descriptor language, detector confirms at run time. **Gate (user):** "spike magnitude ranks spike sections P > O > A" (criterion-level, scoped to the spike set).
...
- **Wiki:** [[motor-metrics-oiah]] Obs.15 (two-channel model) + Hyp.2 rewritten as designed + Hyp.1 channel note + Obs.1/14 amendments; handoff 08-14 updated; index/log updated. Uncommitted — save ritual pending next session review. Resume token: `resume:2026-08-14`.

## [2026-08-15] wiki lint (session-end run #9) | validate
- **Mechanical checks green:** 46 files scanned (reader agent); 0 broken wikilinks, 0 orphans, index drift clean both directions, frontmatter valid (newly edited handoff + [[motor-metrics-oiah]] verified; overview.md `type: overview` = SCHEMA-silent root page, left as-is).
- **Fixed this run:** (1) Obs.12 mislabel "S042 = purple r542" -> **S049** (ground-truth 51-sample: S049 = purple r542 "the hardest"; S042 = orange r464 — different section; row 542 was always correct); (2) handoff "51 sections / 84 rows" -> **85 rows** (keep-S003 arithmetic: 83+2; sample json confirms 85 unique rows); (3) "all 7 currently `idea`" -> Hyp.2 `designed`, 6 others `idea`.
- **Flagged for user (no fix — judgment):** (a) row total 549 ([[motor-model-requirements]]:16, log:227) vs 552 ([[motor-metrics-oiah]], dump-verified `max_row=552`; 550-552 = End/A552-note rows) — "549" is the stale figure; (b) untagged count 460 (Asp.3) vs 464 (requirements, log) — neither matches a dump-derived count, reconcile next session; (c) log:227 "abrupt 5P/3O/1A" (=9) vs Obs.9 "10 rows (P5/O4/A1)" — the 10-row spike set (incl. S010) is operative per user; log figure stands as historical record (append-only).
- **Carried items (unchanged):** rhythm-segmentation.md:134 "production port in progress" vs finger-control.md:8 landed 2026-08-11 (stale line); 08-13-handoff save-state "commit pending" is historical (committed 78406e5, index marks resolved); page cap [[motor-metrics-oiah]] 453 raw / 359 non-empty (soft ~400 substantive — still under, split candidate if the clearing loop adds more).
- **Verdict: lint green** (mechanical), 4 judgment items flagged for human, 2 of 4 with a clear owner (row total, untagged count).
